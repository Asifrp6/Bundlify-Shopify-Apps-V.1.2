(() => {
  ...
  customElements.define("bundlify-bundles", class extends HTMLElement {
    connectedCallback() { queueMicrotask(() => { if (this.isConnected) this.load(); }); }
    ...
    async load() {
      ...
      try {
        ...
        if (this.dataset.product) url.searchParams.set("productId", this.dataset.product);
        ...
        for (const bundle of bundles) {
          ...
          card.append(title);
          list.append(card);
          this.preparePurchase(card, bundle, root, controller.signal, productRequests);
        }
        this.hidden = !bundles.length && this.dataset.editor !== "true";
        this.querySelector("[data-message]").textContent = bundles.length ? "" : "Activate a bundle in Bundlify. Its products must be published and match this product page.";
      } catch {
        if (!this.isConnected || this.controller !== controller) return;
        this.hidden = this.dataset.editor !== "true";
        this.querySelector("[data-message]").textContent = "Unable to load bundles. Check that the app server is running and the app proxy is deployed.";
      } finally { clearTimeout(timeout); }
    }
    async preparePurchase(card, bundle, root, signal, productRequests = new Map()) {
      ...
      try {
        const currency = this.dataset.currency || window.Shopify?.currency?.active;
        if (!currency) throw new Error("Unable to determine your currency. Refresh this page.");
        const formatter = new Intl.NumberFormat(document.documentElement.lang || "en", { style: "currency", currency, currencyDisplay: "code" });
        const money = cents => formatter.format(cents / 100);
        const discount = Number.isInteger(bundle.discount) && bundle.discount >= 0 && bundle.discount <= 100 ? bundle.discount : 0;
        const updatePrices = () => {
          let subtotal = 0;
          let savings = 0;
          for (const entry of entries) {
            const variant = entry.variants.find(v => String(v.id) === entry.select.value);
            const price = Number(variant.price);
            const original = Math.max(price, Number(variant.compare_at_price) || 0);
            subtotal += price;
            ...
            const source = variant.featured_image?.src || entry.details.featured_image || entry.details.images?.[0];
          ...
          if (savings > 0) {
            ...
            was.setAttribute("aria-label", `Original total ${money(subtotal)}`);
        ...
        const products = await Promise.all(bundle.products.map(product => {
          const url = `${root}/products/${encodeURIComponent(product.handle)}.js`;
          if (!productRequests.has(url)) {
            productRequests.set(url, fetch(url, {
              signal: AbortSignal.any([signal, AbortSignal.timeout(10000)]),
            }).then(response => {
              if (!response.ok) throw new Error("Could not load product options. Refresh and try again.");
        ...
        for (const [index, product] of bundle.products.entries()) {
          ...
          const variants = details.variants?.filter(v => v.available && !details.requires_selling_plan && !v.requires_selling_plan) || [];
          if (!variants.length) throw new Error(`${product.title} is unavailable for a one-time bundle purchase.`);
          if (variants.some(v => !Number.isFinite(v.price) || v.price < 0)) throw new Error("Product prices could not be loaded. Refresh and try again.");
          ...
          image.addEventListener("error", () => { image.hidden = true; placeholder.hidden = false; }, { signal });
          ...
          for (const variant of variants) {
            ...
            option.textContent = variant.public_title || variant.title || "Default";
            select.append(option);
          }
          label.append(select);
          label.hidden = variants.length === 1 && variants[0].title === "Default Title";
          info.append(link, prices, label);
          row.append(image, placeholder, info);
          card.insertBefore(row, total);
          selects.push(select);
          entries.push({ select, variants, details, price, was, image, placeholder });
          select.addEventListener("change", updatePrices, { signal });
        }
        if (selects.length < 2) throw new Error("This bundle needs at least two available products.");
        updatePrices();
        ...
        button.addEventListener("click", async () => {
          ...
          try {
            ...
            const sections = drawer?.getSectionsToRender?.().map(section => section.id).slice(0, 5) || [];
            drawer?.setActiveElement?.(button);
            const group = crypto.randomUUID();
            const response = await fetch(`${root}/cart/add.js`, {
              method: "POST", headers: { "Content-Type": "application/json", "Accept": "application/json" },
              body: JSON.stringify({ items: selects.map(select => ({ id: select.value, quantity: 1,
                properties: { _bundlify_bundle: String(bundle.id), _bundlify_group: group },
              })), ...(sections.length ? { sections, sections_url: location.pathname } : {}) }),
              signal: AbortSignal.timeout(15000),
            });
            const result = await response.json();
            if (!response.ok) throw new Error(result.description || result.message || "Could not add the bundle. Check your cart before trying again.");
            ...
            try { await this.openCart(cart.href, result, drawer, sections); }
            catch { message.textContent = "Bundle added. Open your cart to see the updated totals."; }
            button.textContent = buttonLabel;
            button.disabled = false;
    ...
    async openCart(url, result, drawer, sections) {
      if (drawer?.renderContents && sections.length) {
        if (sections.some(id => typeof result.sections?.[id] !== "string")) {
          ...
          const response = await fetch(refresh, { signal: AbortSignal.timeout(10000) });
          if (!response.ok) throw new Error("Cart refresh failed");
          result.sections = await response.json();
        }
        if (sections.some(id => typeof result.sections?.[id] !== "string")) throw new Error("Cart sections unavailable");
        drawer.classList.remove("is-empty");
        drawer.renderContents(result);
        return;
      }
      const response = await fetch(`${url}.js`, { signal: AbortSignal.timeout(10000) });
      if (!response.ok) throw new Error("Cart refresh failed");
      const cart = await response.json();
      const money = cents => new Intl.NumberFormat(document.documentElement.lang || "en", {
        style: "currency", currency: cart.currency, currencyDisplay: "code",
      }).format(cents / 100);
      ...
      dialog.addEventListener("close", () => { dialog.remove(); this.querySelector("button")?.focus(); });
